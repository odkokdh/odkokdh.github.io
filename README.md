...
**2D 도트** 게임으로 **탑뷰** 형식의 게임이며 **로그라이크** 스타일의 게임입니다. 게임의 배경이 되는 지하철에서 플레이어는 여러 곳을 돌아다니며 한정된 자원으로 최대한 오래 살아남는 생존을 목표로 플레이하게 됩니다.

<br>

## **생존**

본 게임은 플레이어가 지하철에서 다가오는 적의 공격을 피하고 처치하며 위험한 오염 구역에 아무런 대비 없이 들어가면 체력이 감소하는 등 모든 플레이 중에는 **생존**을 염두에 두며 플레이를 해야 합니다.

<br>

## **한정된 자원**

기본 무기 또한 단검만 갖고 있어 적의 공격에 주의를 하며 총과 같은 원거리 무기를 얻는다 해도 **한정된 자원**의 문제로 사용해야 할 시기를 잘 생각해야 할 것입니다.
...
## 적
 ...
 |정예 적|<img src="./img/EliteEnemy.png" width =100 heigth = 100>|일반 적보다 체력과 공격력이 높으며 상황에 따라 다양한 공격을 해오는 위험한 적|
...
<br>
 ...
 |이미지|이름|종류|설명|
 |:---:|:---:|:---:|:---:|
 |<img src="./img/MeleeWeapon.png" width = 30 heigth = 100>|단검|근접 무기|근접한 적을 공격할 수 있는 무기|
 |<img src="./img/Pistol.png">|권총|원거리 무기|멀리 떨어져 있는 적을 조준해서 공격할 수 있는 무기, 1발씩 쏠 수 있다.|
 |<img src="./img/SubMachineGun.png">|기관단총|원거리 무기|멀리 떨어져 있는 적을 조준해서 공격할 수 있는 무기, |
 |<img src="./img/medikit.png" width = 30 heigth = 30>|메디킷|소모성 아이템|플레이어의 체력을 회복시켜주는 1회성 아이템|
...
|:---:|:---:|
|이동|입력한 방향키에 따라 이동|
|근접 공격|마우스 좌클릭을 했을 경우 캐릭터가 바라보고 있는 방향으로 현재 장착한 근접 무기의 공격이 이루어짐|
|원거리 공격|마우스 우클릭을 했을 경우 클릭 위치로 현재 장착한 원거리 무기의 공격이 이루어짐|
...
|속성|속성값|설명|비고|
|:---:|:---:|:---:|:---:|
|이동 방향|Vector2|입력한 방향키의 방향에 따라 플레이어가 이동|현재 위치 벡터 * 이동속도|
|이동 속도|8f|플레이어의 이동 속도||
...
|체력|10f|캐릭터의 체력을 나타내며 0이 될 경우 게임 오버가 된다||
...
|체력|25~50f|적의 체력 수치|적 타입의 따라 체력 수치가 다름|
|속도||적의 이동 속도 수치|적 타입의 따라 이동속도 수치가 다름|
...
4. 플레이어의 체력바와 방사능 오염 수치를 나타내는 UI 구현.
...
8. 플레이어는 이동한 방향키에 따라 움직인다. 카메라는 플레이어를 따라간다.

9. 플레이어는 공격을 받거나 방사능 오염 수치가 최대일 경우 체력이 감소한다.

10. 방사능 오염 수치는 오염 구역 진입 시 조금씩 상승한다.

11. 체력과 방사능 오염 수치는 소모성 아이템을 이용해 회복이 가능하다.

12. 특정 아이템을 이용하여면 방사능 오염 수치 상승을 막아준다.

13. 쿨타임이 있는 구르기 기능을 구현해 일정시간 동안 적의 공격을 무시한다.

14. 플레이어는 마우스 좌클릭을 할 경우 근접 공격, 우클릭을 할 경우 원거리 공격이 이루어진다.

15. 원거리 공격은 마우스 좌표에 따라 목표 위치가 달라진다.

16. 메디킷을 보유한 상태에서 H키를 누르면 체력이 회복된다.

17. 근접 무기는 아무 제약 없이 사용 가능하지만, 총기의 경우 소모성 아이템이 필요하다.

18. 적은 일정 범위 내를 배회하며 플레이어가 일정 범위 내에 존재할 경우 플레이어를 추적한다.

19. 적과 플레이어의 거리가 일정 수치로 좁혀질 경우 적은 공격한다.

20. 적을 일정 주기마다 정해진 수만큼 랜덤하게 스폰 시켜주는 스포너 오브젝트가 있다.

21. 생존자 구역에 거래가 가능한 NPC가 있으며 소모성 아이템 거래가 가능하다.

22. 맵의 각 구역을 일반 구역, 생존자 구역, 방사능 오염 구역, 돌연변이 출현 구역으로 나눈다.
