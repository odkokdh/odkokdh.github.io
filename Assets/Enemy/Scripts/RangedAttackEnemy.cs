...
public class RangedAttackEnemy : MonoBehaviour
{
    ...
    bool loopFor; //for문이 한번 실행됬다는 것을 알려주는 불리언
    ...
    void Update()
    {
        ...
        else if (parent.chase == true)
        {
            ...
            //플레이어와 거리를 계산한다
            attackDis = Vector3.Distance(parent.target.transform.position, transform.position);
            ...
            if (attackDis < 3)
            {
                ...
                float dirX = parent.target.transform.position.x - transform.position.x;
                float dirY = parent.target.transform.position.y - transform.position.y;

                //dirX가 음수이고 dirY가 음수면 플레이어는 왼쪽 아래에 위치
                if (dirX < 0 && dirY < 0)
                {
                    transform.position = Vector3.MoveTowards(rigid.position, new Vector3(transform.position.x + 5, transform.position.y + 5), runSpeed * Time.deltaTime);
                }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "Wall")
        {
            ...
            //레이캐스트를 정면으로 각자의 값으로 발사하며 범위내에 Wall이란 Layer값을 가진 것을 발견하면 해당 값을 저장한다.
